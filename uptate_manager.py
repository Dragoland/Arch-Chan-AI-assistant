...
class UpdateManager(QThread):
    ...
    def __init__(self):
        ...
        self.repo_url = "https://api.github.com/repos/tu-usuario/arch-chan/releases/latest"
            ...
            if latest_version:
                ...
                if self.download_update():
                    ...
                    if self.install_update():
                        ...
                        self.update_finished.emit(True, f"Actualizado a v{latest_version}")
                    else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if self.compare_versions(latest_version, self.current_version) > 0:
                    return latest_version, data.get('body', 'Sin información de cambios')
            return None, None
        except Exception as e:
    ...
    def install_update(self):
        ...
        try:
            ...
            backup_file = os.path.join(PROJECT_PATH, "backups", f"pre_update_{datetime.now().strftime('%Y%m%d_%H%M%S')}.ini")
            subprocess.run(["cp", os.path.join(PROJECT_PATH, "config.ini"), backup_file])
